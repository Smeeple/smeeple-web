            ...
            <div class="sm:grid sm:grid-cols-12 sm:gap-3">
                <div class="sm:col-span-12 lg:col-span-10 lg:col-start-2 xl:col-span-8 xl:col-start-3">
                    <div class="cta-group mb-3 flex justify-center">
                        <button :class="tab === 'consumers' ? 'cta-tertiary' : 'cta-tertiary-outline'" class="rounded-br-none rounded-tr-none" @click="tab = 'consumers'">For consumers</button>

                        <button :class="tab === 'experts' ? 'cta-tertiary' : 'cta-tertiary-outline'" class="ml-0 mt-0 rounded-bl-none rounded-tl-none" @click="tab = 'experts'">For experts</button>
                    </div>
                    ...
                    <div :class="tab === 'consumers' ? '' : 'hidden'">
                        ...
                        <ul class="mt-3 list-none overflow-hidden rounded-lg border border-gray">
                            <li v-for="faq in faqsConsumers" :key="faq.id" :class="{ 'border-l-[3px] border-l-green-light bg-white': openAccordion === faq.id, 'bg-gray-lighter': openAccordion !== faq.id }" class="mb-0 cursor-pointer border-t border-gray first:border-t-0 md-max:py-2 md-max:pl-[44px] md-max:pr-2 lg:py-3 lg:pl-[54px] lg:pr-3" @click="toggleAccordion(faq.id)">
                                ...
                                <div v-if="openAccordion === faq.id" class="mt-2 transition-all duration-300" :style="{ height: openAccordion === faq.id ? 'auto' : '0', opacity: openAccordion === faq.id ? '1' : '0' }">
                                    <p :class="{ '-ml-[3px]': openAccordion === faq.id }" class="-indent-[24px]"><span class="font-poppins text-orange-light">A:</span> <span v-html="faq.answer"></span></p>
                                </div>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div :class="tab === 'experts' ? '' : 'hidden'">
                        ...
                        <ul class="mt-3 list-none overflow-hidden rounded-lg border border-gray">
                            <li v-for="faq in faqsExperts" :key="faq.id" :class="{ 'border-l-[3px] border-l-green-light bg-white': openAccordion === faq.id, 'bg-gray-lighter': openAccordion !== faq.id }" class="mb-0 cursor-pointer border-t border-gray first:border-t-0 md-max:py-2 md-max:pl-[44px] md-max:pr-2 lg:py-3 lg:pl-[54px] lg:pr-3" @click="toggleAccordion(faq.id)">
                                ...
                                <div v-if="openAccordion === faq.id" class="mt-2 transition-all duration-300" :style="{ height: openAccordion === faq.id ? 'auto' : '0', opacity: openAccordion === faq.id ? '1' : '0' }">
                                    <p :class="{ '-ml-[3px]': openAccordion === faq.id }" class="-indent-[24px]"><span class="font-poppins text-20 text-orange-light">A:</span> <span v-html="faq.answer"></span></p>
                                </div>
                            </li>
                        </ul>
                    </div>
